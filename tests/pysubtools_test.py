...
class TestCase(unittest.TestCase):
    def test_sif(self):
        ...
        subtitle.save(io.BufferedWriter(
            io.FileIO(tmpfd2, mode='w')), human_time=False)
    ...
    def test_sif_gz(self):
        """Test gzipped 'Subtitle Intermediate Format' loaders and dumpers
        (just wrapped around GzipFile)."""
        # Make a test subtitle with two lines
        subtitle = Subtitle()
        subtitle.add_unit(SubtitleUnit(
            start=15,
    ...
    def test_parsers(self):
        ...
        def format_test(root, suffix, format):
            for filename in (i for i in os.listdir(root)
    ...
    def test_encoding(self):
        """Tests if internal encoder tester works as it should
        (premature IO closures are the concern)"""
...
"""
    ...
    def test_lookup(self):
        """Some encoding python cannot read, we need to make sure
        it won't make a low-level error."""
        f = open('./tests/data/corner/lookup_error.srt', 'rb')
        parser = Parser.from_format('SubRip')

        # This should work now (also, this subtitle has some special
        # chars that without EUC-TW => BIG5-TW would not work)
