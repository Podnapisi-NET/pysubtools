...
class SubRipStateMachine(object):
    ...
    found_header = Event(
        from_states=[unit, unit_text, start], to_state=unit_text)
    ...
    def validate_unit(self):
        ...
        if sequence - previous_seq != 1:
            ...
            self.parser.add_warning(self.current_line_num + 1, 1,
                                    self.fetch_line(
    ...
    def validate_header(self):
        if self.is_unit_text:
            self.parser.add_warning(
                self.current_line_num + 1, 1, self.current_line,
                "Duplicated time information, ignoring.")
            raise self.Skip
        if self.is_start:
            self.fix_sequence_skip()
            self.parser.add_warning(
                self.current_line_num + 1, 1, self.current_line,
                "New unit starts without a sequence.")
        ...
        if '.' in self.current_line:
            ...
            self.parser.add_warning(
                self.current_line_num + 1, col + 1, original,
                'Used dot as decimal separator instead of comma.')
            raise self.Skip
        # Re-check header
        m = self._header.match(self.current_line)
        if m.group(2):
            ...
            self.pause()
            self.parser.add_warning(
                self.current_line_num + 1, column,
                original, 'Header has unrecognized content at the end.')
            raise self.Skip
        ...
        if not start or not end:
            self.parser.add_error(
                self.current_line_num + 1, 1, self.fetch_line(
                    self.current_line_num), "Could not parse timings.")
            raise self.Skip
    ...
    def parse_time(self):
        ...
        def convert(x): return int(x[0]) * 3600 + \
            int(x[1]) * 60 + float(x[2].replace(',', '.'))
    ...
    def insert_text(self):
        ...
        self.temp['data']['lines'] += [i.rstrip()
                                       for i in self.current_line.split('|')]

        # Unknown TAG headers
        if tagged:
            self.parser.add_warning(
                self.current_line_num + 1, 1, self.fetch_line(
                    self.current_line_num), 'Tagged header not fully parsed.')
            raise self.Skip
        ...
        if self.current_state == self.start:
            if self.temp:
                ...
                self.parser.add_warning(
                    self.current_line_num + 1, 1, self.fetch_line(
                        self.current_line_num),
                    "Have empty line before first unit.")
                raise self.Skip
        elif self.is_unit:
            self.parser.add_warning(
                self.current_line_num + 1, 1, self.fetch_line(
                    self.current_line_num),
                "Have empty line between sequence number and timings.")
            raise self.Skip
    ...
    def final_unit(self):
        ...
        if self._missing_line:
            original = self.fetch_line(self.current_line_num)
            self.parser.add_warning(self.current_line_num + 1, len(
                self.current_line), original, 'Missing empty line after unit.')
            raise self.Skip
    ...
    def _parse(self, **kwargs):
        ...
        while True:
            try:
                ...
                self.add_error(machine.current_line_num + 1, 1,
                               machine.current_line, "Unparsable line")
