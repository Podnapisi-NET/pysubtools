...
class MicroDVDParser(Parser):
            ...
            if k.lower() == 'y':
                ...
                for i in v.split(','):
                    ...
                    if i == 'b':
                        ...
                        t['styles']['*']['text-decoration'].append(
                            'line-through')
                    else:
    ...
    def _to_header_dict(self, h):
        ...
        try:
            ...
            self.add_warning(self._current_line_num + 1, line.index(h),
                             line, "It looks like a line header but it's not.")
            return {}
    ...
    def _parse_metadata(self):
        ...
        while self._next_line():
            ...
            if m:
                output.update(self._parse_header(
                    m.group('header'), global_only=True))
    ...
    def _parse(self, fps=None, **kwargs):
        while self._next_line():
            ...
            if not m:
                self.add_error(self._current_line_num + 1, 1,
                               self._current_line, "Could not parse line")
            else:
                if not m.group('text'):
                    self.add_warning(self._current_line_num + 1,
                                     1,
                ...
                h_inherit = [self._to_header_dict(
                    m.groupdict().get('header', ''))]
