...
class ParseError(Exception):
    ...
    def __unicode__(self):
        return
        u"Parse error on line {} at column {} error occurred '{}'".format(
            self.line_number,
            self.column,
            self.description
        )
...
class ParseWarning(ParseError):
    def __unicode__(self):
        return
        "Parse warning on line {} at column {} warning occurred '{}'".format(
            self.line_number,
            self.column,
            self.description
        )
...
class Parser(object):
        ...
        try:
            if isinstance(data, file):
                data = io.BufferedReader(
                    io.FileIO(data.fileno(), closefd=False))
        except NameError:
    ...
    def _can_parse(cls, data):
        """Needs to be reimplemented to quickly check if file seems
        the proper format."""
        raise NotImplementedError

    def _parse(self):
        """
        Parses the file, it returns a list of units in specified format.
        Needs to be implemented by the parser. It can also be a generator
        (yield)
        """
        raise NotImplementedError
    ...
    def parse(self, data=None, encoding=None, language=None, **kwargs):
        """Parses the file and returns the subtitle. Check warnings after
        the parse."""
        if data:
        ...
        for unit in self._parse(**kwargs):
            try:
                ...
                self.add_error(
                    ...
                    1,
                    self._current_line,
                    """Wrongly parsed unit, might be a result of a previous
                    error."""
                )
        return sub
    ...
    def from_data(data, encoding=None, language=None, **kwargs):
        ...
        encoding, encoding_confidence = encodings.detect(
