...
def detect(data, encoding=None, language=None):
    """
    Tries to detect encoding for specified 'data'. Will return a tuple
    (encoding, confidence). Confidence may be None, which means the encoding
    was detected from provided language or encoding hint, or it stumbled
    over a unicode BOM.
    """
    ...
    while True:
        ...
        if not encodings:
            # We lost :(
            raise EncodingError(
                "Could not detect proper encoding", list(tried_encodings))
