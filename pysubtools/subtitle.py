...
class SubtitleUnit(object):
    ...
    def get_moved(self, distance):
        """Same as SubtitleUnit.move, just returns a copy while
        itself is unchanged."""
    ...
    def get_stretched(self, factor):
        """Same as SubtitleUnit.stretch, just returns a copy while
        itself is unchanged."""
        clone = SubtitleUnit(**self.__dict__)
    ...
    if sys.version_info[0] >= 3:  # Python 3
        ...
        def __repr__(self):
            ...
            lines = d.pop('_lines')
            return b"SubtitleUnit({}, {}, {}, {})".format(start, end,
                                                          repr(lines), d)

    def to_dict(self, human_time=True):
        """Returns subtitle unit as a dict
        (with some human readable things)."""
        output = {}
    ...
    def from_dict(cls, input):
        ...
        try:
            lines = [
                i if isinstance(i, unicode) else i.decode('utf-8')
                if isinstance(i, bytes) else SubtitleLine.from_export(i)
                for i in lines
            ]
        except NameError:
            # Python3 compat
            lines = [
                i if isinstance(i, str) else i.decode('utf-8')
                if isinstance(i, bytes) else SubtitleLine.from_export(i)
                for i in lines
            ]
...
class Subtitle(object):
    ...
    def add_unit(self, unit):
        """Adds a new 'unit' and sorts the units. If adding many units,
        use append instead."""
        self.append(unit)
        self.order()
    ...
    def remove(self, unit):
        ...
        if not isinstance(unit, SubtitleUnit):
            raise TypeError(
                "Can remove only SubtitleUnit, you passed '{}'".
                format(type(unit)))
    ...
    def index(self, unit):
        ...
        if not isinstance(unit, SubtitleUnit):
            raise TypeError(
                "Can index only SubtitleUnit, you passed '{}'".
                format(type(unit)))
    ...
    def insert(self, index, unit):
        ...
        if not isinstance(unit, SubtitleUnit):
            raise TypeError(
                "Can add only SubtitleUnit, you passed '{}'".
                format(type(unit)))
    ...
    def from_dict(cls, data):
        ...
        if data is None:
            data = {}
        data = dict(data)
        data['units'] = [SubtitleUnit.from_dict(
            i) for i in data.get('units') or []]
        return cls(**data)
    ...
    def from_file_multi(cls, input):
        """Loads multiple subtitles from file 'input'. It returns
        a generator object."""
        input = prepare_reader(input)
    ...
    def save(self, output, human_time=True, close=True, allow_unicode=True):
        """
        Saves the subtitle in native (YAML) format. If 'output' is
        file object, it will be closed if 'close' set to True after
        save is done.
        """
        try:
        ...
        try:
            if isinstance(output, file):
                output = io.BufferedWriter(
                    io.FileIO(output.fileno()), closefd=close)
        except NameError:
            # No need in Python3
            pass
